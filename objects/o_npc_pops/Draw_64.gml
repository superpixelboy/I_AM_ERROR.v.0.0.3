if (instance_exists(o_text)&&global.convoTag=="Mandy") {
	...
	// === Who's speaking? ===
	var speaker_is_agent = (sprite == s_portrait_null_normal||sprite == s_portrait_null_normal_question||sprite == s_portrait_null_normal_smile);
	var textbox_sprite = speaker_is_agent ? s_textbox_left : s_textbox_right;
...
draw_sprite_stretched(textbox_sprite, 0, 0, 0, display_get_gui_width(), display_get_gui_height());
	...
	if (speaker_is_agent) {
	    draw_sprite_ext(s_portrait_pops, 0, left_x, portrait_y, 1, 1, 0, c_white, 0.5);
